...
def is_valid_key(key):
    if not os.path.exists(KEYS_FILE): return False
...
def make_pair_selection_keyboard(selected_pairs, mode):
    ...
    buttons = []
    for pair in pool:
        label = f" {pair}" if pair in selected_pairs else f" {pair}"
...
def fetch_and_send_news_signals(chat_id, message_id):
    ...
    try:
        ...
        if data.get("status") != "success" or not data.get("signals"):
            bot.send_message(chat_id, "<tg-emoji emoji-id=\"6312054040381367354\"></tg-emoji> <b>      </b>", parse_mode='HTML')
            return
        ...
        for sig in data["signals"]:
            ...
            news_template = (
                ...
                f'<tg-emoji emoji-id="6303243301520678287"></tg-emoji> Manage Risk Properly <tg-emoji emoji-id="6303178164046666974"></tg-emoji>\n\n'
        ...
        credit_text = (
            ...
            f'<tg-emoji emoji-id="6300924843814558816"></tg-emoji> Powered By: {POWERED_BY}</b>'
        )
...
def check_password(message):
    ...
    if is_valid_key(entered_key):
        ...
        bot.send_message(chat_id, '<tg-emoji emoji-id="6066584947039148700"></tg-emoji> <b>Invalid or Used Key! Please try again or contact Admin.</b>', parse_mode='HTML')
...
def admin_genkey(message):
    if message.from_user.id == ADMIN_ID:
        try:
            ...
            bot.send_message(message.chat.id, " <b>Usage:</b> <code>/genkey [amount]</code>\nExample: <code>/genkey 5</code>", parse_mode='HTML')
...
def global_callback_router(call):
    ...
    if state == 'DAYS_SELECT_BACKTEST' and call.data.startswith('b_day_'):
        ...
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(types.InlineKeyboardButton(" BACK", callback_data="back_to_b_days"), types.InlineKeyboardButton(" HOME", callback_data="go_home"))
    ...
    if state == 'FUTURE_MARKET_SELECT' and call.data.startswith('f_m_'):
        ...
        bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text='<tg-emoji emoji-id="6066485535726118367"></tg-emoji> <b>TAP PAIRS FROM GRID TO SELECT / UNSELECT:</b>', reply_markup=keyboard, parse_mode='HTML')
        return
    ...
    if state == 'FUTURE_GRID_SELECTING' and call.data.startswith('toggle_'):
        ...
        display_text = f'<tg-emoji emoji-id="6066485535726118367"></tg-emoji> <b>TAP PAIRS FROM GRID TO SELECT / UNSELECT:</b>\n\n<tg-emoji emoji-id="6311890389242487133"></tg-emoji> <b>Selected:</b> <code>{pairs_formatted}</code>'
    ...
    if state == 'FUTURE_GRID_SELECTING' and call.data == 'pair_selection_done':
        ...
        if not valid_markets:
            bot.answer_callback_query(call.id, text=" Please select at least ONE pair before continuing!", show_alert=True)
            return
...
def global_text_handler(message):
    ...
    if state == 'COLLECTING_BACKTEST':
        if text == '/done':
            if not user_data[chat_id].get('raw_signals'):
                bot.send_message(chat_id, ' <b>No signals received yet.</b>', parse_mode='HTML')
                return
            user_data[chat_id]['state'] = 'DAYS_SELECT_BACKTEST'
            markup = types.InlineKeyboardMarkup(row_width=3)
            markup.add(*[types.InlineKeyboardButton(f" Day {i}", callback_data=f'b_day_{i}') for i in range(1, 8)])
    ...
    if state == 'FUTURE_START_TIME':
        if not re.match(r'^\d{2}:\d{2}$', text):
            bot.send_message(chat_id, " <b>Invalid format! Please enter start time in HH:MM format (e.g. 10:30):</b>", parse_mode='HTML')
            return
    ...
    if state == 'FUTURE_END_TIME':
        if not re.match(r'^\d{2}:\d{2}$', text):
            bot.send_message(chat_id, " <b>Invalid format! Please enter end time in HH:MM format (e.g. 18:45):</b>", parse_mode='HTML')
            return
...
def process_live_signal(chat_id, message_id, pair):
    bot.edit_message_text(chat_id=chat_id, message_id=message_id, 
                          text=f'<tg-emoji emoji-id="6145389934699094865"></tg-emoji>  {pair}\n<tg-emoji emoji-id="6147508869699477866"></tg-emoji>    · ·', parse_mode='HTML')
    ...
    try:
        ...
        signal_template = f"""<b>
   <tg-emoji emoji-id="6300646267940774133"></tg-emoji>  ZEBRONIX LIVE AI  <tg-emoji emoji-id="6300646267940774133"></tg-emoji>


 <tg-emoji emoji-id="6300679098670784062"></tg-emoji>       : {pair}
 <tg-emoji emoji-id="6312351947902952139"></tg-emoji>       : {trend}
 <tg-emoji emoji-id="6312054040381367354"></tg-emoji> iron  : {direction}
 <tg-emoji emoji-id="6311921227107671342"></tg-emoji>   : 
 <tg-emoji emoji-id="6312196117899517195"></tg-emoji>       : {entry_time}
 <tg-emoji emoji-id="6300924843814558816"></tg-emoji>    :  {strength}% <tg-emoji emoji-id="6303181741754424089"></tg-emoji>
 <tg-emoji emoji-id="6303030189538417585"></tg-emoji>  :    



 <tg-emoji emoji-id="6301088640982326387"></tg-emoji>     : {support}
 <tg-emoji emoji-id="6145389934699094865"></tg-emoji>  : {resistance}


<tg-emoji emoji-id="6131977683841589337"></tg-emoji>  : {OWNER_USERNAME}<tg-emoji emoji-id="6312042143321957539"></tg-emoji>
</b>"""
...
def execute_future_generation(chat_id, message_id, filter_days):
    ...
    output_text += "<b></b>\n"
    if market_mode == "BLACKOUT":
        output_text += f'<b><tg-emoji emoji-id="6132052287423522342"></tg-emoji>Total: {len(filtered)}</b>\n\n<b>           :</b>\n<b><tg-emoji emoji-id="6210635890994192211"></tg-emoji> 00:54        Down </b>\n<b><tg-emoji emoji-id="6210635890994192211"></tg-emoji> 00:54        Up </b>\n\n'
    else:
