...
def global_callback_router(call):
    ...
    if state == 'FUTURE_GRID_SELECTING' and call.data == 'pair_selection_done':
        ...
        if not valid_markets:
            bot.answer_callback_query(call.id, text=" Please select at least ONE target pair to stream calculation data analytics!", show_alert=True)
            return
    ...
    if state == 'COLLECTING_SIGNALS':
        if text == '/done':
            if not user_data[chat_id].get('raw_signals'):
                bot.send_message(chat_id, ' <b>No raw metrics context stream compiled yet.</b>', parse_mode='HTML')
                return
            user_data[chat_id]['state'] = 'SELECTING_MTG'
...
def execute_future_generation(chat_id, message_id, filter_days):
    ...
    if market_mode == "BLACKOUT":
        output_text += (
            ...
            "<b>           :</b>\n"
            '<b><tg-emoji emoji-id="6172215346947166980"></tg-emoji> 00:54        <tg-emoji emoji-id="6066652700148243688"></tg-emoji> Down </b>\n'
            '<b><tg-emoji emoji-id="6172215346947166980"></tg-emoji> 00:54        <tg-emoji emoji-id="6066511369954402964"></tg-emoji> Up </b>\n\n'
...
def broadcast_handler(message):
    if message.from_user.id == ADMIN_ID:
        ...
        if not msg_text:
            bot.send_message(message.chat.id, '<tg-emoji emoji-id="6311936890853402623"></tg-emoji> <b>Please provide message context.</b>', parse_mode='HTML')
            return
