...
# --- New Advanced Custom Pure Python AI Filter Engine ---
def advanced_custom_ai_filter(signals_list, days):
    ...
    if days in [1, 2, 3]:
        # আগে, পরে এবং মাঝখান থেকে কিছু অংশ কেটে একদম ক্লিয়ার করা হবে
        if total >= 8:
        ...
        for sig in middle_pool:
            ...
            if last_time is None:
                ...
                diff = int((curr_time - last_time).total_seconds() / 60)
                # মাঝখানের অংশ ক্লিয়ার ও ব্যালেন্স রাখার জন্য একটি স্মার্ট গ্যাপ ফিল্টার
                if diff >= random.choice([6, 9, 14]):
                    filtered_signals.append(sig)
                    last_time = curr_time
    else:
        # Day 4, 5, 6, 7 এর জন্য লজিক
        # শুরুর দিকের কিছু সিগনাল রিমুভ করা হবে (Fast remove)
        if total >= 5:
        ...
        last_time = None
        # বাধ্যতামূলক গ্যাপ অপশন: ৪, ৬, ৮, ১২ মিনিট
    ...
    # যদি সম্পূর্ণ খালি হয়ে যায় তবে সুরক্ষার জন্য মূল লিস্ট থেকে ২/৩ টি সিগনাল ব্যাকআপ রাখবে
...
def global_callback_router(call):
    ...
    if call.data == 'btn_market_live':
        ...
        ai_welcome = (
            ...
            'The engine uses custom Python Data Filters based on Strategy Depth Days Selection.\n\n'
    ...
    # --- New Core Handler for AI FILTER Day Buttons Selection ---
...
def global_text_handler(message):
    ...
    if state == 'AI_COLLECTING_SIGNALS':
        if text == '/process':
            ...
            # AI ফিল্টারের জন্য Day Strategy Depth নির্বাচন ইন্টারফেস
        ...
        new_signals = parse_raw_signals(text)
        user_data[chat_id]['raw_signals'].extend(new_signals)
        bot.send_message(chat_id, f"<tg-emoji emoji-id='6066872327595892055'>🤑</tg-emoji> <b>Added {len(new_signals)} signals. Send /process to configure strategy depth.</b>", parse_mode='HTML')
        return
