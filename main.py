...
OTC_PAIRS_PLAIN = [
    'TRUUSD-OTC', 'MANUSD-OTC', 'BCHUSD-OTC', 'AXSUSD-OTC', 'ATOUSD-OTC', 'TONUSD-OTC', 'MSFT-OTC', 'LTCUSD-OTC',
    'USDNGN-OTC', 'USDCOP-OTC', 'USDBRL-OTC', 'USDMXN-OTC', 'USDPHP-OTC', 'USDINR-OTC', 'USDZAR-OTC', 'USDBDT-OTC',
    'USDPKR-OTC', 'USDARS-OTC', 'USDDZD-OTC', 'USDIDR-OTC', 'USDEGP-OTC', 'CADCHF-OTC', 'NZDUSD-OTC', 'NZDCAD-OTC',
    'AUDCAD-OTC', 'AUDJPY-OTC', 'AUDNZD-OTC', 'GBPJPY-OTC', 'EURCAD-OTC', 'AUDCHF-OTC', 'CHFJPY-OTC', 'GBPUSD-OTC',
    'EURUSD-OTC', 'USDJPY-OTC', 'USDCAD-OTC', 'USDCHF-OTC', 'ETCUSD-OTC', 'BNBUSD-OTC', 'ETHUSD-OTC', 'SOLUSD-OTC'
]

REAL_PAIRS_PLAIN = [
    'EURUSD', 'EURGBP', 'AUDJPY', 'CADJPY', 'AUDUSD', 'AUDCAD', 'USDJPY', 'EURCAD', 'AUDCHF', 'GBPAUD',
    'GBPCAD', 'GBPJPY', 'GBPUSD', 'EURAUD', 'CHFJPY', 'GBPCHF', 'USDCHF', 'EURCHF', 'USDCAD', 'EURJPY'
]
...
def generate_future_signals(valid_markets, start_time, end_time, mode, filter_days):
    ...
    try:
        ...
        if filter_days <= 3:
            hash_threshold, gap_modifier = 35, 2
        elif filter_days <= 5:
            hash_threshold, gap_modifier = 60, 4
        else:
            hash_threshold, gap_modifier = 82, 7
...
def global_callback_router(call):
    ...
    # ইউজার অবজেক্ট ক্র্যাশ এড়াতে গ্যারান্টি চেক
    if chat_id not in user_data:
        user_data[chat_id] = {'raw_signals': [], 'selected_pairs': [], 'state': 'MAIN_MENU'}
    ...
    if call.data == 'btn_live_signal_mode':
        ...
        bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text='<tg-emoji emoji-id=\"6302799249146911743\">📊</tg-emoji> <b>Selected pair for live signal:</b>', reply_markup=keyboard, parse_mode='HTML')
        return
    ...
    if call.data == 'btn_future_mode':
        ...
        return
    # --- ফিক্সড: গ্লোবাল বাটন ট্র্যাকিং (যাতে ডেটা ওভারফ্লো বা স্টেট লক না হয়) ---
    if call.data.startswith('b_day_'):
        selected_day = call.data.split('_')[2]
        bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text='<pre><tg-emoji emoji-id="5440410042773824003">🔗</tg-emoji> Running Original Backtest Engine...</pre>', parse_mode='HTML')
        ...
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(types.InlineKeyboardButton("⬅️ BACK", callback_data="back_b_d"), types.InlineKeyboardButton("🏠 HOME", callback_data="go_home"))
        bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text=header_text + body_text, reply_markup=markup, parse_mode='HTML')
        return
    # কলব্যাক স্ট্রিং ছোট করা হয়েছে যেন টেলিগ্রামের ৬৪ বাইট লিমিট ওভারফ্লো না করে
    if call.data == "back_b_d":
...
def process_live_signal(chat_id, message_id, pair):
    bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=f'<tg-emoji emoji-id="6145389934699094865">🔈</tg-emoji> 𝙰𝚗𝚊𝚕𝚢𝚣𝚒𝚗げる {pair}\n<tg-emoji emoji-id="6147508869699477866">😕</tg-emoji> 𝚂𝚌𝚊𝚗𝚗𝚒𝚗𝚐 𝚖𝚊𝚛𝚔𝚎𝚝 𝚍𝚊𝚝𝚊 · ·', parse_mode='HTML')
    ...
    try:
        ...
        signal_template = f"""<b> ╔═══════════════╗ <tg-emoji emoji-id="6300646267940774133">▶️</tg-emoji> ZEBRONIX LIVE AI <tg-emoji emoji-id="6300646267940774133">▶️</tg-emoji> ╚═══════════════╝ 

┏━━━━━━━━━━━━━━━━━━━━ ┃ <tg-emoji emoji-id="6300679098670784062">⚙️</tg-emoji> 𝙰𝚜𝚜𝚎𝚝 : {pair} ┃ <tg-emoji emoji-id="6312351947902952139">🚀</tg-emoji> 𝚃𝚛𝚎𝚗𝚍 : {trend} ┃ <tg-emoji emoji-id="6312054040381367354">🕯</tg-emoji> 𝙳ir𝚎𝚌𝚝𝚒on : {direction} ┃ <tg-emoji emoji-id="6311921227107671342">⏰</tg-emoji> 𝚃𝚒𝚖𝚎𝚏𝚛𝚊𝚖𝚎 : 𝙼𝟷 ┃ <tg-emoji emoji-id="6312196117899517195">🔔</tg-emoji> 𝙴𝚗𝚝𝚛𝚢 : {entry_time} ┃ <tg-emoji emoji-id="6300924843814558816">☄️</tg-emoji> 𝚂𝚝𝚛𝚎𝚗𝚐𝚝𝚑 : 𝙷𝚒𝚐𝚑 {strength}% <tg-emoji emoji-id="6303181741754424089">📊</tg-emoji> ┃ <tg-emoji emoji-id="6303030189538417585">➕</tg-emoji> 𝙼𝚃𝙶 : 𝙼𝚊𝚛𝚝𝚒𝚗𝚐𝚊𝚕𝚎 𝙾𝚗𝚎 𝚂𝚝𝚎𝚙 ┗━━━━━━━━━━━━━━━━━━━━

┏━━━━━━━━━━━━━━━━┓ ┃ <tg-emoji emoji-id="6301088640982326387">😋</vor-emoji> 𝚂𝚞𝚙𝚙𝚘𝚛𝚝 : {support} ┃ <tg-emoji emoji-id="6145389934699094865">🔈</tg-emoji> 𝚁𝚎𝚜𝚒𝚜𝚝𝚊𝚗𝚌𝚎 : {resistance} ┗━━━━━━━━━━━━━━━━┛ ┏━━━━━━━━━━━━━━━━━┓ ┃ <tg-emoji emoji-id="6131977683841589337">👑</tg-emoji> 𝙾𝚆𝙽𝙴𝚁 : {OWNER_USERNAME}<tg-emoji emoji-id="6312042143321957539">✅</tg-emoji> ┗━━━━━━━━━━━━━━━━━┛</b>"""
