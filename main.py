...
GEMINI_API_KEY = 'AQ.Ab8RN6KLCHz7qK_sTDDaj3n8mxjjhExmTAshUr9aWzanxpxZw'
    ...
    try:
        ...
        if str(chat_id) not in users and str(chat_id).strip() != "":
            with open(USER_FILE, 'a') as f:
                f.write(f"{chat_id}\n")
            print(f"[SYSTEM] New user saved to database: {chat_id}")
    ...
    try:
        ...
        for pair in valid_markets:
            ...
            while current_slot <= end_slot:
                ...
                seed = f"{pair}-{time_str}-{mode}-{fixed_date_str}-{filter_days}-{trend}-{rsi:.1f}-{pair_offset}"
...
# --- Smart Automated Gap Generator Logic ---
...
def kimi_ai_filter(signals_list):
    ...
    prompt = (
        f"You are an advanced Binary Options Trading AI. Analyze the following signals and select only the highest precision setups.\n"
        f"Strictly output the chosen signals formatted exactly like this, with no extra conversational text or markdown blocks:\n"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if final_gap_signals:
                final_output = "\n".join([f"M1;{s['asset']};{s['time']};{s['direction']}" for s in final_gap_signals])
                return final_output
            else:
                return "AI Processed but no signals passed the automated gap rules."
        else:
            return f"Gemini API Link Interrupted (Status Code: {response.status_code})"
    except Exception as e:
        return f"Gemini Cloud System Processing Failure: {str(e)}"
...
def global_callback_router(call):
    ...
    if call.data == 'btn_market_live':
        ...
        ai_welcome = (
            ...
            'The engine uses real-time candle data parsing combined with <b>RSI, Support/Resistance & Trend Filter.</b>\n\n'
    ...
    if state == 'FUTURE_SIGNAL_SELECTING' and call.data == 'pair_selection_done':
        ...
        if market_mode != "BLACKOUT":
            ...
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton('🟢 CALL ONLY', callback_data='f_d_1'),
                types.InlineKeyboardButton('🔴 PUT ONLY', callback_data='f_d_2'),
                types.InlineKeyboardButton('🟣 BOTH SIGNAL', callback_data='f_d_3'),
...
def global_text_handler(message):
    ...
    if state == 'AI_COLLECTING_SIGNALS':
        ...
        new_signals = parse_raw_signals(text)
        user_data[chat_id]['raw_signals'].extend(new_signals)
        bot.send_message(chat_id, f"<tg-emoji emoji-id='6066872327595892055'>🤑</tg-emoji> <b>Added {len(new_signals)} signals. Send /process to run ZEBRONIX AI.</b>", parse_mode='HTML')
        return
